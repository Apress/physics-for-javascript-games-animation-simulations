...
function calcForce(obj, num) {
  ...
  if (num - Math.floor(num / rows) * rows > 0) {
    ...
    // current particle is in first row, no particle above, just use current particle's position and velocity
    centerUp = obj.pos2D;
    veloUp = obj.velo2D;
  }
  ...
  if (num + 1 - Math.floor((num + 1) / rows) * rows > 0) {
    ...
    // current particle is in last row, no particle below, just use current particle's position and velocity
  ...
  if (num > rows - 1) {
    ...
    // current particle is in first column, no particle to the left, just use current particle's position and velocity
  ...
  if (num < rows * (cols - 1)) {
    ...
    // current particle is in last column, no particle to the right, just use current particle's position and velocity
  ...
  var extensionDown = displDown.subtract(
    displDown.unit().multiply(springLength)
  );
  var extensionLeft = displLeft.subtract(
    displLeft.unit().multiply(springLength)
  );
  var extensionRight = displRight.subtract(
    displRight.unit().multiply(springLength)
  );
  ...
  var windForce = Forces.linearDrag(
    -1 * kWind,
    windVelocity.subtract(obj.velo2D)
  );
