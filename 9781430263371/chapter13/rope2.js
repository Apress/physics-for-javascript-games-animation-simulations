...
function calcForce(obj, num) {
  ...
  var extensionPrev = displPrev.subtract(
    displPrev.unit().multiply(springLength)
  );
  var extensionNext = displNext.subtract(
    displNext.unit().multiply(springLength)
  );
  ...
  if (num == balls.length - 1) {
    ...
    if (pull == true) {
      ...
      // if mouse is not pressed hold last particle fixed
