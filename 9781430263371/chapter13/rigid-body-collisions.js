...
function checkWallBounce(obj) {
  ...
  for (var i = 0; i < obj.vertices.length; i++) {
    if (obj.pos2D.add(obj.vertices[i].rotate(obj.rotation)).y >= wall.p1.y) {
      if (testCollision == false) {
        ...
        // that means one vertex is already touching
        //doSomethingClever();
        j2 = i;
  ...
  if (testCollision == true) {
    obj.y +=
      obj.pos2D.add(obj.vertices[j].rotate(obj.rotation)).y * -1 + wall.p1.y;
    if (testCollision2 == true) {
      // reposition the other vertex too to prevent it from sinking
      obj.y +=
        obj.pos2D.add(obj.vertices[j2].rotate(obj.rotation)).y * -1 + wall.p1.y;
      testCollision2 == false;
    }
    ...
    var impulse =
      (-(1 + cr) * vp1.dotProduct(normal)) /
      (1 / obj.mass + (rp1Xnormal * rp1Xnormal) / obj.im);
...
function checkObjectCollision(obj1, obj2) {
  ...
  if (
    obj1.pos2D.subtract(obj2.pos2D).length() <
    obj1.maxVertex + obj2.maxVertex
  ) {
    ...
    for (var i = 0; i < obj1.vertices.length; i++) {
      ...
      if (j == obj2.vertices.length) {
        ...
        for (j = 0; j < obj2.vertices.length; j++) {
          ...
          var vectCheck = displ_vertices.project(
            obj2.sides[j].rotate(obj2.rotation)
          );
          var norm2 = displ_vertices.subtract(vectCheck);
          if (
            Math.abs(
              Vector2D.angleBetween(
                obj1.vertices[i].rotate(obj1.rotation).multiply(-1),
                norm2
              )
            ) < anglRef
          ) {
        ...
        var impulse =
          (-(1 + cr) * vr.dotProduct(normal)) /
          (invm1 + invm2 + rp1Xn * rp1Xn * invI1 + rp2Xn * rp2Xn * invI2);
