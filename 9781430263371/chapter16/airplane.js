...
var Ixx = 20000,
  ...
  Izz = 160000;
var I = new Matrix3D(
  new Vector3D(Ixx, 0, 0),
  new Vector3D(0, Iyy, 0),
  new Vector3D(0, 0, Izz)
);
var Iinv = new Matrix3D(
  new Vector3D(1 / Ixx, 0, 0),
  new Vector3D(0, 1 / Iyy, 0),
  new Vector3D(0, 0, 1 / Izz)
);
...
function setupObjects() {
  ...
  airplane = new THREE.Mesh(
    new THREE.CubeGeometry(400, 100, 100),
...
function startControl(evt) {
  ...
  if (evt.keyCode == 37) {
    // left arrow: roll pozive wrt x axis, roll to the left
    if (alphaAl < alphaAlMax) {
      // maximum aileron angle
      alphaAl += alphaAlInc;
    }
  }
  if (evt.keyCode == 39) {
    // right :roll negative wrt x axix, roll to the right
    if (alphaAl > -alphaAlMax) {
      // minimum aileron angle
      alphaAl += -alphaAlInc;
    }
  }
  ...
  if (evt.keyCode == 88) {
    // x key : airplane nose to right
    if (alphaRd > -alphaRdMax) {
      // minimum rudder angle
      alphaRd += -alphaRdInc;
    }
  }
  if (evt.keyCode == 90) {
    // z key ; airplane nose to left
    if (alphaRd < alphaRdMax) {
...
function rotateVector(v, q) {
  // rotate v (a Vector3D object) by quaternion q and return rotated Vector3D
...
function v3D(vec3) {
  // takes a THREE Vector3 and returns a Vector3D version
  return new Vector3D(vec3.x, vec3.y, vec3.z);
}
function v3(vec3D) {
  // takes a Vector3D and returns a THREE Vector3 version
...
function controlRotation(obj) {
  // hack to impose stability
