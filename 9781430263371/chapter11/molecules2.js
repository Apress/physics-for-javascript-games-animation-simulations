...
function init() {
  ...
  var wall2 = new Wall(
    ...
    new Vector2D(canvas.width, 0)
  );
  wall2.draw(context_bg);
  walls.push(wall2);
  var wall3 = new Wall(
    new Vector2D(0, canvas.height),
    new Vector2D(canvas.width, canvas.height)
  );
...
function onMouseMove(evt) {
  var dvec = new Vector2D(
    evt.clientX - balls[numBalls - 1].x,
    evt.clientY - balls[numBalls - 1].y
  );
...
function checkWallBounce(obj) {
  ...
  for (var i = 0; i < walls.length && hasHitAWall == false; i++) {
    ...
    var test =
      Math.abs(proj1) < wdir.length() && Math.abs(proj2) < wdir.length();
